...
import { StyleSheet, Text, View, TextInput, TouchableOpacity, Image,TouchableWithoutFeedback,Dimensions } from 'react-native'
...
export default function Login({navigation}) {
  ...
  const handleClick = async () => {
    ...
    const resp = await axios.post("http://172.20.10.6:8000/api/signin", newUser);
    if(resp.data.error) 
             ...
             <Image source={require("../assets/logo-no-background.png")} style={styleAuth.logoStyle} />
              ...
              style={styleAuth.loginInput} />
                ...
                <View style={{flex:1 ,flexDirection:'row',justifyContent:'center',
                    ...
                    Already Joined? {" "}
                    <Text style={{ color: '#EA9937', alignItems:'center'}} onPress={() => navigation.navigate("SignUp")}>
                        Sign Up
                    </Text>
                </Text>
                </View>
                <TouchableOpacity onPress={()=>navigation.navigate('ForgetPassword')}>
                <Text style={{ color:'#EA9937',fontSize: 12, textAlign: 'center'}}>Forget Password?</Text>
              </TouchableOpacity> 
               </View>
</View>
</TouchableWithoutFeedback>
