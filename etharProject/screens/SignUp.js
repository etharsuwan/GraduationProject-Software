...
export default function SignUp({navigation}) {
    ...
    const handleSubmit = async () => {
        if(name === '' ||email === '' || phone===''||city===''||license===''|| password === '') {
            alert('Please fill all fields');
            return;
        }
        const newUser= {
            name, email,phone,city,license, password
        };
        const resp = await axios.post("http://172.20.10.6:8000/api/signup", newUser);
        if(resp.data.error) 
        ...
        <PhoneInput
                ...
                />
                            ...
                            <View style={{ marginHorizontal: 24,marginBottom:20 }}>
                                <Text style={{ fontSize: 14, color: '#EA9945'}}>Work License</Text>
                                <TextInput
                    ...
                    <Text style={{ fontSize: 14, textAlign: 'center' }}>
                            Already Joined? {" "}
                            <Text style={{ color: '#EA9937', alignItems:'center'}} onPress={() => navigation.navigate("Login")}>
                                Sign In
                            </Text>
                        </Text>
                        </View>    
